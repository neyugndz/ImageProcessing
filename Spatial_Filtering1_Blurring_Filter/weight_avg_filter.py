...
#Filter Image using Weighted Smoothing method
def weighted_filter(img):
    ...
    #Define weight matrix for weighted average filter
    avg_filter = [
            [1, 2, 1],
            [2, 4, 2],
            [1, 2, 1]    
    ...
    for y in range(1, height - 1):
        for x in range(1, width - 1):
            total = 0
            #Sum up all neighboring pixels in weighted avg
            for i in range(-1,2):
                for j in range(-1,2):
                    pixel = pixels[x+i, y+j]
                    weight = avg_filter[j+1][i+1]
                    total += pixel * weight
            filtered_pixels[x, y] = total // sum  
...
#Show negative image
img_filtered = weighted_filter(img)
plt.subplot(2, 3, 3)
...
plt.title('Grayscale Histogram of Weighted Filtered Image')
plt.xlabel('Pixel value')
plt.ylabel('Frequency')
